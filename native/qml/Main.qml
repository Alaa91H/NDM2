...
ApplicationWindow {
    id: window
    width: 1440; height: 900; minimumWidth: 1080; minimumHeight: 680; visible: true
    title: "NDM2"
    color: dark ? "#0C1628" : "#EDF2F8"
    readonly property bool dark: settingsService.theme !== "light"
    ...
    function statusFor(sectionName) { return sectionName === "active" ? "downloading" : sectionName === "queued" ? "queued" : sectionName === "completed" ? "completed" : sectionName === "failed" ? "error" : "" }
    function titleFor(sectionName) { return sectionName === "all" ? qsTr("All downloads") : sectionName === "active" ? qsTr("Active downloads") : sectionName === "queued" ? qsTr("Queue") : sectionName === "completed" ? qsTr("Completed") : sectionName === "failed" ? qsTr("Failed") : sectionName === "scheduled" ? qsTr("Scheduled") : sectionName === "history" ? qsTr("History") : qsTr("Categories") }
    function filterItem(name, status, category, queueId) { var match = statusFor(section); var search = searchText.toLowerCase(); if (match.length && status !== match) return false; if (section === "scheduled" && queueId.length === 0) return false; return !search.length || name.toLowerCase().indexOf(search) >= 0 || category.toLowerCase().indexOf(search) >= 0 }
    ...
    Connections { target: taskController
    ...
    RowLayout { anchors.fill: parent; spacing: 0
        Rectangle { id: nav; Layout.fillHeight: true; Layout.preferredWidth: 246; color: window.sidebar; border.color: window.dark ? "#1D2F4B" : "#DCE5F0"; border.width: 1
            ColumnLayout { anchors.fill: parent; anchors.margins: 15; spacing: 8
                ...
                NavItem { label: qsTr("Queued"); glyph: "≡"; selected: window.section === "queued"; count: taskController.downloads.countForStatus("queued"); Layout.fillWidth: true; onClicked: window.section = "queued" }
                ...
                NavItem { label: qsTr("Scheduled"); glyph: "◷"; selected: window.section === "scheduled"; Layout.fillWidth: true; onClicked: window.section = "scheduled" }
                NavItem { label: qsTr("Categories"); glyph: "◇"; selected: window.section === "categories"; Layout.fillWidth: true; onClicked: window.section = "categories" }
                NavItem { label: qsTr("History"); glyph: "↶"; selected: window.section === "history"; Layout.fillWidth: true; onClicked: window.section = "history" }
                Item { Layout.fillHeight: true }
                Rectangle { Layout.fillWidth: true; Layout.preferredHeight: 68; radius: 10; color: taskController.connected ? "#112E35" : "#35202B"; border.color: taskController.connected ? "#22515B" : "#663446"
                    RowLayout { anchors.fill: parent; anchors.margins: 10; spacing: 9; Rectangle { width: 8; height: 8; radius: 4; color: taskController.connected ? "#4FD3A4" : "#FF7385" } ColumnLayout { Layout.fillWidth: true; spacing: 1; Label { text: taskController.connected ? qsTr("Core connected") : qsTr("Core unavailable"); color: "#D8E7EC"; font.pixelSize: 11; font.weight: Font.DemiBold } Label { Layout.fillWidth: true; text: taskController.connected ? qsTr("Loopback daemon") : taskController.lastError; color: "#8FABBB"; font.pixelSize: 9; elide: Text.ElideRight } } }
                }
            }
        }
        Rectangle { Layout.fillWidth: true; Layout.fillHeight: true; color: window.bg
            ColumnLayout { anchors.fill: parent; spacing: 0
                Rectangle { Layout.fillWidth: true; Layout.preferredHeight: 76; color: window.surface; border.color: window.dark ? "#1D2F4B" : "#DCE5F0"; border.width: 1
                    RowLayout { anchors.fill: parent; anchors.leftMargin: 28; anchors.rightMargin: 28; spacing: 14
                        ColumnLayout { Layout.fillWidth: true; spacing: 1; Label { text: window.titleFor(window.section); color: window.text; font.pixelSize: 21; font.weight: Font.DemiBold } Label { text: taskController.downloads.count + qsTr(" tasks managed by the NOVA core"); color: window.muted; font.pixelSize: 11 } }
                        TextField { id: search; Layout.preferredWidth: 270; placeholderText: qsTr("Search downloads  Ctrl+F"); selectByMouse: true; onTextChanged: window.searchText = text; background: Rectangle { radius: 9; color: window.dark ? "#0F1B2E" : "#F0F4F9"; border.color: search.activeFocus ? "#4A90F5" : "transparent" } leftPadding: 13; rightPadding: 13 }
                        ToolButton { text: "↻"; font.pixelSize: 19; onClicked: taskController.refresh(); ToolTip.text: qsTr("Refresh from core"); ToolTip.visible: hovered }
                ...
                Rectangle { Layout.fillWidth: true; Layout.preferredHeight: 62; color: window.bg
                ...
                Rectangle { Layout.fillWidth: true; Layout.fillHeight: true; Layout.leftMargin: 20; Layout.rightMargin: 20; Layout.bottomMargin: 16; color: window.surface; radius: 12; border.color: window.dark ? "#1E304B" : "#DCE4EF"
                    ColumnLayout { anchors.fill: parent; spacing: 0
                        Rectangle { Layout.fillWidth: true; Layout.preferredHeight: 44; color: window.dark ? "#122038" : "#F5F8FC"; radius: 12
                        ...
                        ListView { id: list; Layout.fillWidth: true; Layout.fillHeight: true; clip: true; model: taskController.downloads; spacing: 2; visible: count > 0
                            delegate: DownloadRow { required property string downloadId; required property string name; required property string status; required property string fileType; required property double progress; required property double speed; required property double sizeBytes; required property double downloadedBytes; required property double eta; required property int connections; required property string category; required property string queueId; required property string errorMessage; visible: window.filterItem(name, status, category, queueId); selected: taskController.selectedId === downloadId; compact: settingsService.density === "compact"; onActivated: taskController.selectedId = downloadId }
                            ScrollBar.vertical: ScrollBar { policy: ScrollBar.AsNeeded }
                        }
                        EmptyState { Layout.fillWidth: true; Layout.fillHeight: true; visible: list.count === 0; title: taskController.connected ? qsTr("No downloads in this view") : qsTr("Waiting for the NOVA core"); subtitle: taskController.connected ? qsTr("Create a download and the core will provide its live state here.") : taskController.lastError; onActionRequested: addDialog.open() }
                    }
                }

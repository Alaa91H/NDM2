...
interface Props {
  ...
  onFactoryReset: () => void;
  activeSubTab?: 'bridge' | 'diagnostics' | 'backup' | 'advanced' | 'logging';
  onChangeSubTab?: (tab: 'bridge' | 'diagnostics' | 'backup' | 'advanced' | 'logging') => void;
}
...
export const DiagnosticsAndSystem: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-6 text-left animate-in fade-in duration-200">
      ...
      {subTab === 'logging' && <LoggingPanel settings={settings} updateSetting={updateSetting} />}
    </div>
  );
};

const LoggingPanel: React.FC<{ settings: AppSettings; updateSetting: Props['updateSetting'] }> = ({
  settings,
  updateSetting,
}) => {
  const t = useI18n();
          ...
          >
            ...
            {logs.map((entry, idx) => (
              <div
                ...
                className="px-2 py-0.5 hover:bg-[var(--bg-hover)] border-b border-[var(--border-color)]/30 flex gap-2"
              >
                ...
                <span className="text-[var(--text-primary)] flex-1 min-w-0 break-all">{entry.message}</span>
              </div>
            ))}
          </div>
        </div>
